class Spree::ReimbursementType::StoreCredit < Spree::ReimbursementType
  ...
  class << self
    ...
    def create_creditable(reimbursement, unpaid_amount)
      ...
      Spree::StoreCredit.new(user: reimbursement.order.user, amount: unpaid_amount, category: category, created_by: Spree::StoreCredit.default_created_by, memo: "Refund for uncreditable payments on order #{reimbursement.order.id}")
    end
